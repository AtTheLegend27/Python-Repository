...
def tail_points(opponent_score):
    ...
    t = opponent_score // 10 % 10
    o = opponent_score % 10 
    return add(mul(abs(sub(t, o)) , 2) , 1)
    # END PROBLEM 2
...
def next_perfect_square(x):
    if perfect_square(x) is True:
        return int(mul(realsqrt(x)+1, realsqrt(x)+1))
# END PROBLEM 4
...
def play(strategy0, strategy1, update,
    ...
    while max(score0, score1) < goal:
        if who == 0:
            ...
            #take_turn(num_rolls, score1, dice) 
            score0 = update(num_rolls, score0, score1, dice) 
        else:
            num_rolls = strategy1(score1, score0)
            #take_turn(num_rolls, score0, dice)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    myscore = 0
    opponentscore = 0
    x = strategy(myscore, opponentscore)
    while myscore < goal:
        while opponentscore < goal:
            if x == strategy(myscore, opponentscore):
                opponentscore += 1
            else:
                return False
        myscore += 1
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average(*args):
        ...
        while counter > 0:
            totalvalue += original_function(*args)
            counter -= 1
        return totalvalue / total_samples
    return average
    # END PROBLEM 8
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    z = tail_points(opponent_score)
    s = tail_strategy(score, opponent_score, threshold, num_rolls)
    if z >= threshold:
        return 0 
    elif perfect_square(z+score):
        if (next_perfect_square(z+score)) - score >= threshold:
            return 0
    else:
        return num_rolls
    # END PROBLEM 11
