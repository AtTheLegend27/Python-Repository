...
def pingpong(n):
    ...
    def thing(x, y, z):
        if n == y:
            return x
        elif y % 8 ==0 or num_eights(y) > 0:
            return thing(x - z, y + 1, - z)
        else: 
            return thing(x + z, y + 1, z)
    return thing( 1, 1, 1)
...
def count_coins(change):
    ...
    True
    """
    def thingtwo(change, markcoin):
        if change == 0:
            return 1
        elif change < 0 or markcoin == None: 
            return 0
        
        keepcount = thingtwo(change - markcoin, markcoin)
        nokeepcount = thingtwo(change, next_larger_coin(markcoin))
        return keepcount + nokeepcount 
    return thingtwo(change, 1)
